...
string add(string s1, string s2) {
    ...
    string finum; // Инициализируем строку для формирования итогового числа

    // Формируем результат, преобразуя массив чисел обратно в строку
    for (int i = part; i >= 0; i--) {
        finum += to_str(c[i]); // Добавляем каждую часть результата в итоговую строку
    }

    // Удаляем ведущие нули из итоговой строки
    while (finum.substr(0, 1) == "0") {
        finum.erase(finum.find("0"), 1); // Удаляем первый встреченный ноль
    }

    return finum; // Возвращаем итоговую строку
}
